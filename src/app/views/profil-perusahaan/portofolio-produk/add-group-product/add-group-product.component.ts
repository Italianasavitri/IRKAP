...
export class AddGroupProductComponent implements OnInit, AfterViewInit, AfterContentInit {
  PortofolioProduk: string[];
  daftarPortofolioProduk = [];
  ...
  constructor(private groupproductService: GroupProductService) {
    ...
    this.PortofolioProduk =  groupproductService.getPortofolioProduk();
    this.groupproductService.getPortofolio().subscribe(respRole=>{
      this.daftarPortofolioProduk = respRole.d.list;
    })
    ...
    if (!this.isEdit) {
      ...
      this.groupproductService.save(this.role).subscribe(resp => {
        ...
        if (success) {
          ...
          notify({message: 'Portofolio Produk berhasil disimpan', position: {my: 'center top', at: 'center top'}},
           'success', 3000);
          this.hide();
        } else {
      ...
      this.groupproductService.update(this.role).subscribe(resp => {
       ...
       success = true;
        ...
        if (success) {
          this.options.message = 'Portofolio Produk updated';
          notify(this.options, 'success', 3000);
          this.hide();
        } else {
