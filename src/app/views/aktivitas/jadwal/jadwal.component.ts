...
export class JadwalComponent implements AfterViewInit {
  ...
  constructor(
    ...
    @Inject(JadwalService) private jadwalService: JadwalService
  ) {
    this.search = {
      jadwalid: '',
      deskripsi: '',
    ...
    pagination(){
      ...
      this.jadwalService.getLimit(this.offset,this.limitVal)
      .subscribe(resp => {
        console.log(resp);
        this.gridDataSource = resp.d.list;
  ...
  refresh() {
    this.jadwalService.getAll().subscribe(resp => {
      this.gridDataSource = resp.d.list;
    }, err => {
      console.log(err);
    })
  ...
  getRoleData() {
    ...
    this.jadwalService.getAll().subscribe(resp => {
      this.jadwalService.getAllRoleAuth().subscribe(respAuth => {
  ...
  searching() {
    this.jadwalService.getByName(this.search.tipe_jadwal).subscribe(resp => {
      this.jadwalService.getAllRoleAuth().subscribe(respAuth => {
        resp.forEach((value, index) => {
          const menus = [];
          const menu = respAuth.filter(element => {
      ...
      if (this.search.tipe_jadwal !== '') {
        this.gridDataSource = resp.d.list;
        //this.gridDataSource = resp.filter(role => role.roleName === this.search.tipe_jadwal).filter(data => data.activationCode === 'Y');
      } else {
        this.gridDataSource = resp.filter(data => data.activationCode === 'Y');
      }
    });
  ...
  advSearch() {
    this.jadwalService.getByData(this.search).subscribe(resp => {
      this.jadwalService.getAllRoleAuth().subscribe(respAuth => {
        resp.forEach((value, index) => {
          const menus = [];
          const menu = respAuth.filter(element => {
  ...
  onDeleteConf() {
    this.jadwalService.getById(this.target).subscribe(role => {
      this.jadwalService.delete(role.d).subscribe(resp => {
        // this.getRoleData();
        notify({
          closeOnClick: true,
          displayTime: 3000,
