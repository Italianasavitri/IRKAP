...
import {Menu, TargetTahunan, KategoriAktif} from '../target-tahunan.model';
...
import { DxiItemComponent } from 'devextreme-angular/ui/nested/item-dxi';
 ...
 export class AddTargetComponent implements OnInit, AfterViewInit, AfterContentInit {
   @Input() isEdit;
   @Input() isDetail;
   @Input() editItem;
   @Input() addVisible;
   ...
   constructor(private targettahunanService: TargetTahunanService) {
     //this.simpleProducts = visiService.getSimpleProducts();
     this.Tahun =  targettahunanService.getTahun();
     this.targettahunanService.getRjpp().subscribe(resp=>{
       this.daftarRjpp = resp.d.list;
     })
     this.targettahunanService.getSasaran().subscribe(resp=>{
   ...
   ngOnInit() {
    if (this.isEdit || this.isDetail) {
      ...
      this.targettahunanService.getById(this.editItem).subscribe(respRole => {
        console.log(respRole);
        console.log(this.editItem);
        const nilai: string = respRole.d.tid;
        this.role = {
          tid: respRole.d.tid,
  ...
  checkBoxToggled(e) {
   ...
   onSaveConf() {
     ...
     if (!this.isEdit) {
       this.role.tid = null;
       this.targettahunanService.save(this.role).subscribe(resp => {
         console.log(resp);
         success = true;
         this.menuTree.forEach(menuItem => {
           if (menuItem.read || menuItem.write) {
             ...
             this.targettahunanService.saveRoleAuth({
               read: read,
               write: write,
       ...
       this.targettahunanService.update(this.role).subscribe(resp => {
         ...
         this.menuTree.forEach(menuItem => {
           if (menuItem.read || menuItem.write) {
             ...
             if (typeof menuItem.authId === 'undefined') {
              ...
              this.targettahunanService.updateRoleAuth({
             ...
             if (typeof menuItem.authId !== 'undefined') {
               this.targettahunanService.deleteRoleAuth(menuItem.authId).subscribe(() => { }, () => {
                 success = false;
               });
             }
           }
