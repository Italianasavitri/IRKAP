...
import {Search} from './strategis.model';
...
@Component({
  selector: 'app-strategis',
  templateUrl: './strategis.component.html',
  styleUrls: ['./strategis.component.scss'],
  providers: [StrategisService]
})
export class StrategisComponent implements AfterViewInit {
  ...
  constructor(
    ...
    @Inject(StrategisService) private strategisService: StrategisService
  ) {
    this.search = {
      visi_id: '',
      visi: '',
  ...
  pagination(){
    ...
    this.strategisService.getLimit(this.offset,this.limitVal)
    .subscribe(resp => {
      console.log(resp);
      this.gridDataSource = resp.d.list;
  ...
  refresh() {
    this.strategisService.getAll().subscribe(resp => {
      this.gridDataSource = resp.d.list;
    }, err => {
      console.log(err);
    })
  ...
  getRoleData() {
    ...
    this.strategisService.getAll().subscribe(resp => {
      this.strategisService.getAllRoleAuth().subscribe(respAuth => {
  ...
  searching() {
    this.strategisService.getByName(this.search.visi).subscribe(resp => {
      this.strategisService.getAllRoleAuth().subscribe(respAuth => {
        resp.forEach((value, index) => {
          const menus = [];
          const menu = respAuth.filter(element => {
  ...
  advSearch() {
    this.strategisService.getByData(this.search).subscribe(resp => {
      this.strategisService.getAllRoleAuth().subscribe(respAuth => {
        resp.forEach((value, index) => {
          const menus = [];
          const menu = respAuth.filter(element => {
  ...
  onDeleteConf() {
    this.strategisService.getById(this.target).subscribe(role => {
      this.strategisService.delete(role.d).subscribe(resp => {
        // this.getRoleData();
        notify({
          closeOnClick: true,
          displayTime: 3000,
