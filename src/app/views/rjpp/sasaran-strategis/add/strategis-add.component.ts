...
import {Menu, Strategis, KategoriAktif} from '../strategis.model';
...
import { DxiItemComponent } from 'devextreme-angular/ui/nested/item-dxi';
 ...
 @Component({
   selector: 'app-add-strategis',
   templateUrl: './strategis-add.component.html',
   providers: []
 })
 export class AddStrategisComponent implements OnInit, AfterViewInit, AfterContentInit {
   @Input() isEdit;
   @Input() isDetail;
   @Input() editItem;
   @Input() addVisible;
   ...
   constructor(private strategisService: StrategisService) {
     this.Tahun =  strategisService.getTahun();
     this.strategisService.getRjpp().subscribe(resp=>{
       this.daftarRjpp = resp.d.list;
       console.log(this.daftarRjpp);
     })
     this.strategisService.getSatuan().subscribe(resp=>{
       this.daftarSatuan = resp.d.list;
       console.log(this.daftarSatuan);
     })
     this.strategisService.getStrategis().subscribe(resp=>{
      this.daftarSstrategis = resp.d.list;
    })
    this.strategisService.getObyektif().subscribe(resp=>{
   ...
   ngOnInit() {
    if (this.isEdit || this.isDetail) {
      ...
      this.strategisService.getById(this.editItem).subscribe(respRole => {
  ...
  ngAfterContentInit() {
   ...
   onSaveConf() {
     ...
     if (!this.isEdit) {
       this.role.sid = null;
       this.role.jenis_sasaran = "Strategis";
       this.strategisService.save(this.role).subscribe(resp => {
         console.log(resp);
         success = true;
       ...
       this.strategisService.update(this.role).subscribe(resp => {
