...
// @Component({
//   selector: 'app-obyektif',
//   templateUrl: './obyektif.component.html',
//   styleUrls: ['./obyektif.component.scss']
// })
// export class ObyektifComponent implements OnInit {
...
import {Search} from './obyektif.model';
...
@Component({
  ...
  templateUrl: './obyektif.component.html',
  styleUrls: ['./obyektif.component.scss'],
  providers: [ObyektifService]
})
export class ObyektifComponent implements AfterViewInit {
  ...
  constructor(
    ...
    @Inject(ObyektifService) private obyektifService: ObyektifService
  ) {
    this.search = {
      visi_id: '',
      visi: '',
  ...
  pagination(){
    ...
    this.obyektifService.getLimit(this.offset,this.limitVal)
    .subscribe(resp => {
      console.log(resp);
      this.gridDataSource = resp.d.list;
  ...
  refresh() {
    this.obyektifService.getAll().subscribe(resp => {
      this.gridDataSource = resp.d.list;
    }, err => {
      console.log(err);
    })
  ...
  getRoleData() {
    ...
    this.obyektifService.getAll().subscribe(resp => {
      this.obyektifService.getAllRoleAuth().subscribe(respAuth => {
  ...
  searching() {
    this.obyektifService.getByName(this.search.visi).subscribe(resp => {
      this.obyektifService.getAllRoleAuth().subscribe(respAuth => {
        resp.forEach((value, index) => {
          const menus = [];
          const menu = respAuth.filter(element => {
  ...
  advSearch() {
    this.obyektifService.getByData(this.search).subscribe(resp => {
      this.obyektifService.getAllRoleAuth().subscribe(respAuth => {
        resp.forEach((value, index) => {
          const menus = [];
          const menu = respAuth.filter(element => {
  ...
  onDeleteConf() {
    this.obyektifService.getById(this.target).subscribe(role => {
      this.obyektifService.delete(role.d).subscribe(resp => {
        // this.getRoleData();
        notify({
          closeOnClick: true,
          displayTime: 3000,
