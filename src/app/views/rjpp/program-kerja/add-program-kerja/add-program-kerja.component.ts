...
import { ReportingRoutingModule } from 'app/views/reporting/reporting-routing.module';
...
export class AddProgramKerjaComponent implements OnInit, AfterViewInit, AfterContentInit {
  ...
  constructor(private programkerjaService: ProgramKerjaService) {
    ...
    this.programkerjaService.getRjpp().subscribe(resp=>{
      this.daftarRjpp = resp.d.list;
    })
    this.programkerjaService.getSasaran().subscribe(resp=>{
      this.daftarSasaran = resp.d.list;     
    })
    this.programkerjaService.getPortofolioProduk().subscribe(resp=>{
      this.daftarPortofolioProduk = resp.d.list;
    })
  ...
  ngOnInit() {
   if (this.isEdit || this.isDetail) {
     ...
     this.programkerjaService.getById(this.editItem).subscribe(respRole => {
       ...
       const nilai: string = respRole.d.prkid;
       this.role = {
