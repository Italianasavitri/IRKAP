...
import {Search} from './entri-non-rkap.model';
...
@Component({
  selector: 'app-entri-non-rkap',
  templateUrl: './entri-non-rkap.component.html',
  styleUrls: ['./entri-non-rkap.component.scss'],
  providers: [EntriNonRkapService]
})
export class EntriNonRkapComponent implements AfterViewInit {
  ...
  constructor(
    ...
    @Inject(EntriNonRkapService) private entrinonrkapService: EntriNonRkapService
  ) {
    this.search = {
      rjppid: '',
      sid: '',
    ...
    pagination(){
      ...
      this.entrinonrkapService.getLimit(this.offset,this.limitVal)
      .subscribe(resp => {
        console.log(resp);
  ...
  refresh() {
    this.entrinonrkapService.getAll().subscribe(resp => {
      this.gridDataSource = resp.d.list;
    }, err => {
      console.log(err);
    })
  ...
  getRoleData() {
    ...
    this.entrinonrkapService.getAll().subscribe(resp => {
      this.entrinonrkapService.getAllRoleAuth().subscribe(respAuth => {
  ...
  searching() {
    this.entrinonrkapService.getByName(this.search.rkapid).subscribe(resp => {
      this.entrinonrkapService.getAllRoleAuth().subscribe(respAuth => {
        resp.forEach((value, index) => {
          const menus = [];
          const menu = respAuth.filter(element => {
  ...
  advSearch() {
    this.entrinonrkapService.getByData(this.search).subscribe(resp => {
      this.entrinonrkapService.getAllRoleAuth().subscribe(respAuth => {
        resp.forEach((value, index) => {
          const menus = [];
          const menu = respAuth.filter(element => {
  ...
  onDeleteConf() {
    this.entrinonrkapService.getById(this.target).subscribe(role => {
      this.entrinonrkapService.delete(role.d).subscribe(resp => {
        // this.getRoleData();
        notify({
          closeOnClick: true,
          displayTime: 3000,
