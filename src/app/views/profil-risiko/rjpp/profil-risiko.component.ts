...
import {Search} from './profil-risiko.model';
...
@Component({
  selector: 'app-profil-risiko',
  templateUrl: './profil-risiko.component.html',
  styleUrls: ['./profil-risiko.component.scss'],
  providers: [ProfilRisikoService]
})

export class ProfilRisikoRJPPComponent implements AfterViewInit {
  ...
  constructor(
    ...
    @Inject(ProfilRisikoService) private profilrisikoService: ProfilRisikoService
  ) {
    this.search = {
      riskid: null,
      risiko: '',
    ...
    //this.profilrisikoService.getAll()
    //.subscribe(resp => {
    //  console.log(resp);
  ...
  pagination(){
    ...
    this.profilrisikoService.getLimit(this.offset,this.limitVal)
    .subscribe(resp => {
      console.log(resp);
      this.gridDataSource = resp.d.list;
  ...
  refresh() {
    this.profilrisikoService.getAll().subscribe(resp => {
      this.gridDataSource = resp.d.list;
    }, err => {
      console.log(err);
    })
  ...
  getRoleData() {
    ...
    this.profilrisikoService.getAll().subscribe(resp => {
      this.profilrisikoService.getAllRoleAuth().subscribe(respAuth => {
  ...
  addNewButton() {
    ...
    const $customButton = $('<div id="addNewRole">').dxButton({
      icon: 'add',
      text: 'Tambah Profil Risiko RJPP',
      type: 'default',
      onClick: function () {
        ini.isAdd = true;
  ...
  searching() {
    this.profilrisikoService.getByName(this.search.risiko).subscribe(resp => {
      this.profilrisikoService.getAllRoleAuth().subscribe(respAuth => {
        resp.forEach((value, index) => {
          const menus = [];
          const menu = respAuth.filter(element => {
      ...
      if (this.search.risiko !== '') {
        this.gridDataSource = resp.filter(role => role.roleName === this.search.risiko).filter(data => data.activationCode === 'Y');
      } else {
        this.gridDataSource = resp.filter(data => data.activationCode === 'Y');
      }
    });
  ...
  advSearch() {
    this.profilrisikoService.getByData(this.search).subscribe(resp => {
      this.profilrisikoService.getAllRoleAuth().subscribe(respAuth => {
        resp.forEach((value, index) => {
          const menus = [];
          const menu = respAuth.filter(element => {
  ...
  onDeleteConf() {
    this.profilrisikoService.getById(this.target).subscribe(role => {
      this.profilrisikoService.delete(role.d).subscribe(resp => {
        // this.getRoleData();
        notify({
          closeOnClick: true,
          displayTime: 3000,
