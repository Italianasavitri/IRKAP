...
import { Departemen, KategoriAktif } from './departemen.model';
...
@Injectable()
export class DepartemenService {
  private resourceUrlRole = this.a.SERVER_URL + '/system/Departemen';
  ...
  getById(id: any): Observable<any> {
    ...
    return this.http.get(this.resourceUrlRole + '/retrieve?token=' + token + '&deptid=' + id)
  }
  ...
  save(data: Departemen): Observable<any> {
    const username = localStorage.getItem('username');
    const token = localStorage.getItem('token');
  ...
  update(data: Departemen): Observable<any> {
    const username = localStorage.getItem('username');
    const token = localStorage.getItem('token');
    const today = new Date().toISOString().slice(0, 10);
    console.log(data);
  ...
  delete(data: any): Observable<any> {
    ...
    const token = localStorage.getItem('token');
    return this.http.get(this.resourceUrlRole + '/delete?token=' + token + '&deptid=' + data.deptid)
  }
