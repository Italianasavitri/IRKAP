...
import {Search} from './departemen.model';
...
// }
export class DepartemenComponent implements AfterViewInit {
  ...
  constructor(
    ...
    @Inject(DepartemenService) private departemenService: DepartemenService
  ) {
    this.search = {
      deptid: null,
      dept_name: '',
  ...
  pagination(){
    ...
    this.departemenService.getLimit(this.offset,this.limitVal)
    .subscribe(resp => {
      console.log(resp);
      this.gridDataSource = resp.d.list;
  ...
  refresh() {
    this.departemenService.getAll().subscribe(resp => {
      this.gridDataSource = resp.d.list;
    }, err => {
      console.log(err);
    })
  ...
  getRoleData() {
    ...
    this.departemenService.getAll().subscribe(resp => {
      this.departemenService.getAllRoleAuth().subscribe(respAuth => {
  ...
  searching() {
    this.departemenService.getByName(this.search.dept_name).subscribe(resp => {
      this.departemenService.getAllRoleAuth().subscribe(respAuth => {
        resp.forEach((value, index) => {
          const menus = [];
          const menu = respAuth.filter(element => {
      ...
      if (this.search.dept_name !== '') {
        this.gridDataSource = resp.filter(role => role.roleName === this.search.dept_name).filter(data => data.activationCode === 'Y');
      } else {
        this.gridDataSource = resp.filter(data => data.activationCode === 'Y');
      }
    });
  ...
  advSearch() {
    this.departemenService.getByData(this.search).subscribe(resp => {
      this.departemenService.getAllRoleAuth().subscribe(respAuth => {
        resp.forEach((value, index) => {
          const menus = [];
          const menu = respAuth.filter(element => {
  ...
  onDeleteConf() {
    this.departemenService.getById(this.target).subscribe(role => {
      this.departemenService.delete(role.d).subscribe(resp => {
        // this.getRoleData();
        notify({
          closeOnClick: true,
          displayTime: 3000,
