...
import {Menu, Departemen, KategoriAktif} from '../departemen.model';
...
@Component({
  selector: 'app-add-depertemen',
  templateUrl: './add-depertemen.component.html',
  providers: []
})
// export class AddDepertemenComponent implements OnInit {
...
// }
export class AddDepertemenComponent implements OnInit, AfterViewInit, AfterContentInit {
  @Input() isEdit;
  @Input() isDetail;
  @Input() editItem;
  @Input() addVisible;
  ...
  constructor(private departemenService: DepartemenService) {
    this.simpleProducts = departemenService.getSimpleProducts();
  ...
  ngOnInit() {
   if (this.isEdit || this.isDetail) {
     ...
     this.departemenService.getById(this.editItem).subscribe(respRole => {
       console.log(respRole);
       console.log(this.editItem);
       const nilai: string = respRole.d.deptid;
       this.role = {
         deptid: respRole.d.deptid,
         parent_deptid: respRole.d.parent_deptid,
         //urutan: parseInt(nilai),
         dept_name: respRole.d.dept_name,
         manager_id: respRole.d.manager_id,
         tgl_mulai: respRole.d.tgl_mulai,
         tgl_akhir: respRole.d.tgl_akhir,
         isactive: respRole.d.isactive,
         level: respRole.d.level,
         cost_center: respRole.d.cost_center,
       };
 ...
 checkBoxToggled(e) {
  ...
  onSaveConf() {
    ...
    const nilai: number = this.role.deptid;
    this.role.deptid = nilai;
    let success = false;
    if (!this.isEdit) {
      this.role.deptid = null;
      this.departemenService.save(this.role).subscribe(resp => {
        console.log(resp);
        success = true;
        this.menuTree.forEach(menuItem => {
          if (menuItem.read || menuItem.write) {
            ...
            this.departemenService.saveRoleAuth({
              read: read,
              write: write,
        ...
        if (success) {
          ...
          notify({message: 'Departemen berhasil disimpan', position: {my: 'center top', at: 'center top'}},
           'success', 3000);
          this.hide();
        } else {
      ...
      this.departemenService.update(this.role).subscribe(resp => {
       ...
       success = true;
        ...
        this.menuTree.forEach(menuItem => {
          if (menuItem.read || menuItem.write) {
            ...
            if (typeof menuItem.authId === 'undefined') {
             ...
             this.departemenService.updateRoleAuth({
            ...
            if (typeof menuItem.authId !== 'undefined') {
              this.departemenService.deleteRoleAuth(menuItem.authId).subscribe(() => { }, () => {
                success = false;
              });
            }
          }
        ...
        if (success) {
          this.options.message = 'Departemen updated';
          notify(this.options, 'success', 3000);
          this.hide();
        } else {
