...
import {Menu, PlanMitigasi, KategoriAktif} from '../plan-mitigasi.model';
...
@Component({
  selector: 'app-add-plan-mitigasi',
  templateUrl: './add-plan-mitigasi.component.html',
  providers: []
})
// export class AddPlanMitigasiComponent implements OnInit {
...
// }
export class AddPlanMitigasiComponent implements OnInit, AfterViewInit, AfterContentInit {
  @Input() isEdit;
  @Input() isDetail;
  @Input() editItem;
  @Input() addVisible;
  ...
  constructor(private planmitigasiService: PlanMitigasiService) {
    this.simpleProducts = planmitigasiService.getSimpleProducts();
  ...
  ngOnInit() {
   if (this.isEdit || this.isDetail) {
     ...
     this.planmitigasiService.getById(this.editItem).subscribe(respRole => {
       console.log(respRole);
       console.log(this.editItem);
       const nilai: string = respRole.d.mitigasiid;
       this.role = {
         mitigasiid: respRole.d.mitigasiid,
 ...
 checkBoxToggled(e) {
  ...
  onSaveConf() {
    ...
    const nilai: number = this.role.mitigasiid;
    this.role.mitigasiid = nilai;
    let success = false;
    if (!this.isEdit) {
      this.role.mitigasiid = null;
      this.planmitigasiService.save(this.role).subscribe(resp => {
        console.log(resp);
        success = true;
        this.menuTree.forEach(menuItem => {
          if (menuItem.read || menuItem.write) {
            ...
            this.planmitigasiService.saveRoleAuth({
              read: read,
              write: write,
        ...
        if (success) {
          ...
          notify({message: 'Plan Mitigasi berhasil disimpan', position: {my: 'center top', at: 'center top'}},
           'success', 3000);
          this.hide();
        } else {
      ...
      this.planmitigasiService.update(this.role).subscribe(resp => {
       ...
       success = true;
        ...
        this.menuTree.forEach(menuItem => {
          if (menuItem.read || menuItem.write) {
            ...
            if (typeof menuItem.authId === 'undefined') {
             ...
             this.planmitigasiService.updateRoleAuth({
            ...
            if (typeof menuItem.authId !== 'undefined') {
              this.planmitigasiService.deleteRoleAuth(menuItem.authId).subscribe(() => { }, () => {
                success = false;
              });
            }
          }
        ...
        if (success) {
          this.options.message = 'Plan Mitigasi updated';
          notify(this.options, 'success', 3000);
          this.hide();
        } else {
